...
# Now to add Troubles w/ 4% decay 
dta_nir_sub_lmr %>% 
  lm(lmr ~ as.factor(age) * (yr_pre_38 + impr_phase + yr_since_imp + trbls_4pc), 
     data = .) -> mod_lmr_02
...
dta_nir_sub_lmr %>% 
  lm(lmr ~ as.factor(age) * (yr_pre_38 + impr_phase + yr_since_imp + yr_since_imp2 + trbls_4pc), 
     data = .) -> mod_lmr_04
...
# So, model 4 has the worst AIC, but also has the most random looking pattern of residual scatter. 
...
dta_nir_sub_lmr %>% 
  modelr::add_predictions(mod_lmr_04, var = "pred_active") %>%
  dplyr::select(year, age, lmr, pred_active) -> preds_trbls

dta_nir_sub_lmr %>% 
  mutate(trbls_4pc = 0) -> newdat 

broom::augment(mod_lmr_04, newdata = newdat) %>% 
